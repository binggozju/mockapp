...
curl -X POST -H "Accept: application/json" -H "Content-type: application/json" -d "{\"name\": \"Zhengzhou\", \"province\": \"HeNan\"}" $HOST/city 2> /dev/null

# select
#curl -X GET -H "Accept: application/json" $HOST/city/1 2> /dev/null
#curl -X GET -H "Accept: application/json" $HOST/city/list 2> /dev/null
...
# join select
#curl -X GET  -H "Accept: application/json" $HOST/person/list 2> /dev/null
